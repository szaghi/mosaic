...
def _mock_client(get_return=None, post_return=None, get_side_effect=None):
    ...
    Returns a ``(client_cls, mock_client)`` tuple.  ``client_cls`` is the
    replacement for ``httpx.Client`` (use as the *new* argument in
    ``patch("httpx.Client", client_cls)``).  ``mock_client`` is the fake
    client instance whose ``.get`` / ``.post`` can be inspected for
    ``call_args``.
    """
...
class TestArxivSource:
    ...
    def test_parses_paper_fields(self):
        cls, mc = _mock_client(get_return=_mock_get(text=_ARXIV_XML))
    ...
    def test_doi_fallback_to_arxiv_doi(self):
        xml = _ARXIV_XML.replace("<arxiv:doi>10.48550/arXiv.1706.03762</arxiv:doi>", "")
        cls, mc = _mock_client(get_return=_mock_get(text=xml))
        with patch("httpx.Client", cls):
            papers = self._source().search("attention")
...
class TestSemanticScholarSource:
    ...
    def test_parses_paper_fields(self):
        cls, mc = _mock_client(get_return=_mock_get(json_data=_SS_JSON))
        with patch("httpx.Client", cls):
            papers = self._source().search("attention")
        assert len(papers) == 1
        p = papers[0]
...
class TestEuropePMCSource:
    ...
    def test_parses_paper_fields(self):
        cls, mc = _mock_client(get_return=_mock_get(json_data=_EPMC_JSON))
        with patch("httpx.Client", cls):
    ...
    def test_year_range_appended_to_query(self):
        ...
        cls, mc = _mock_client(get_return=_mock_get(json_data={"resultList": {"result": []}}))
        with patch("httpx.Client", cls):
            self._source().search("RNA", filters=f)
        query = mc.get.call_args.kwargs["params"]["query"]
...
class TestDoajSource:
    ...
    def test_parses_paper_fields(self):
        cls, mc = _mock_client(get_return=_mock_get(json_data=_DOAJ_JSON))
        with patch("httpx.Client", cls):
    ...
    def test_author_filter_in_query(self):
        ...
        cls, mc = _mock_client(get_return=_mock_get(json_data={"results": []}))
        with patch("httpx.Client", cls):
            self._source().search("test", filters=f)
        url = str(mc.get.call_args.args[0])
        assert "Alice" in url
...
class TestOpenAlexSource:
    ...
    def test_parses_paper_fields(self):
        cls, mc = _mock_client(get_return=_mock_get(json_data=_OPENALEX_JSON))
        with patch("httpx.Client", cls):
            papers = self._source().search("attention")
        assert len(papers) == 1
        p = papers[0]
...
class TestBASESource:
    ...
    def test_parses_paper_fields(self):
        cls, mc = _mock_client(get_return=_mock_get(json_data=_BASE_JSON))
        with patch("httpx.Client", cls):
            papers = self._source().search("attention")
        assert len(papers) == 1
        p = papers[0]
    ...
    def test_no_pdf_url_when_not_oa(self):
        ...
        cls, mc = _mock_client(get_return=_mock_get(json_data={"response": {"docs": [doc]}}))
        with patch("httpx.Client", cls):
            papers = self._source().search("attention")
...
class TestCORESource:
    ...
    def test_parses_paper_fields(self):
        cls, mc = _mock_client(get_return=_mock_get(json_data=_CORE_JSON))
        with patch("httpx.Client", cls):
            papers = self._source().search("attention")
        assert len(papers) == 1
        p = papers[0]
...
class TestNASAADSSource:
    ...
    def test_parses_paper_fields(self):
        cls, mc = _mock_client(get_return=_mock_get(json_data=_NASA_ADS_JSON))
        with patch("httpx.Client", cls):
            ...
            "url": [
                {
                    "format": "html",
                    "platform": "web",
                    "value": "https://link.springer.com/article/10.1007/s10462-023-10466-6",
                },
                {
                    "format": "pdf",
                    "platform": "web",
                    "value": "https://link.springer.com/content/pdf/10.1007/s10462-023-10466-6.pdf",
                },
            ],
        }
    ]
...
class TestSpringerAPISource:
    ...
    def test_parses_paper_fields(self):
        cls, mc = _mock_client(get_return=_mock_get(json_data=_SPRINGER_API_JSON))
        with patch("httpx.Client", cls):
            papers = self._source().search("attention")
        assert len(papers) == 1
        p = papers[0]
    ...
    def test_field_title_uses_title_prefix(self):
        f = SearchFilters(field="title")
        cls, mc = _mock_client(get_return=_mock_get(json_data={"records": []}))
        with patch("httpx.Client", cls):
            self._source().search("attention", filters=f)
        params = mc.get.call_args.kwargs["params"]
        assert params["q"].startswith('title:"')
...
class TestZenodoSource:
    ...
    def test_parses_paper_fields(self):
        cls, mc = _mock_client(get_return=_mock_get(json_data=_ZENODO_JSON))
        with patch("httpx.Client", cls):
            papers = self._source().search("attention")
        assert len(papers) == 1
        p = papers[0]
    ...
    def test_field_title_uses_title_prefix(self):
        f = SearchFilters(field="title")
        cls, mc = _mock_client(get_return=_mock_get(json_data={"hits": {"hits": []}}))
        with patch("httpx.Client", cls):
            self._source().search("attention", filters=f)
        params = mc.get.call_args.kwargs["params"]
        assert params["q"].startswith("title:")
...
class TestCrossrefSource:
    ...
    def test_parses_paper_fields(self):
        cls, mc = _mock_client(get_return=_mock_get(json_data=_CROSSREF_JSON))
        with patch("httpx.Client", cls):
            papers = self._source().search("attention")
        assert len(papers) == 1
        p = papers[0]
...
class TestHALSource:
    ...
    def test_parses_paper_fields(self):
        cls, mc = _mock_client(get_return=_mock_get(json_data=_HAL_JSON))
        with patch("httpx.Client", cls):
...
class TestIEEEXploreSource:
    ...
    def test_parses_paper_fields(self):
        cls, mc = _mock_client(get_return=_mock_get(json_data=_IEEE_JSON))
        with patch("httpx.Client", cls):
    ...
    def test_field_title_uses_title_prefix(self):
        f = SearchFilters(field="title")
        cls, mc = _mock_client(get_return=_mock_get(json_data={"articles": []}))
        with patch("httpx.Client", cls):
            self._source().search("deep learning", filters=f)
        params = mc.get.call_args.kwargs["params"]
...
class TestDBLPSource:
    ...
    def test_parses_paper_fields(self):
        cls, mc = _mock_client(get_return=_mock_get(json_data=_DBLP_JSON))
        with patch("httpx.Client", cls):
            papers = self._source().search("attention")
        assert len(papers) == 1
        p = papers[0]
...
class TestPubMedSource:
    ...
    def _mock_two_step(self, esearch=_PUBMED_ESEARCH, esummary=_PUBMED_ESUMMARY):
        """Context manager patching httpx.Client for the two-step esearch+esummary flow."""
        cls, mc = _mock_client(
            get_return=_mock_get(json_data=esearch),
            post_return=_mock_get(json_data=esummary),
        )
        with patch("httpx.Client", cls):
            yield mc
    ...
    def test_max_results_capped_at_10000(self):
        ...
        assert mc.get.call_args.kwargs["params"]["retmax"] == 10_000
...
class TestPMCSource:
    ...
    def test_uses_db_pmc_in_esearch(self):
        ...
        assert mc.get.call_args.kwargs["params"]["db"] == "pmc"

    def test_uses_db_pmc_in_esummary(self):
        cls, mc = _mock_client(
            get_return=_mock_get(json_data=_PMC_ESEARCH),
            post_return=_mock_get(json_data=_PMC_ESUMMARY),
        )
        with patch("httpx.Client", cls):
            self._source().search("single-cell")
        assert mc.post.call_args.kwargs["data"]["db"] == "pmc"
    ...
    def test_journal_filter_appended_to_query(self):
        ...
        with patch("httpx.Client", cls):
            self._source().search("RNA", filters=f)
        assert '"Nature"[ta]' in mc.get.call_args.kwargs["params"]["term"]
    ...
    def test_field_title_uses_ti_tag(self):
        ...
        assert mc.get.call_args.kwargs["params"]["term"] == "splicing[ti]"
    ...
    def test_field_abstract_uses_ab_tag(self):
        ...
        with patch("httpx.Client", cls):
            self._source().search("splicing", filters=f)
        assert mc.get.call_args.kwargs["params"]["term"] == "splicing[ab]"
    ...
    def test_raw_query_overrides_term(self):
        ...
        with patch("httpx.Client", cls):
            self._source().search("anything", filters=f)
        assert mc.get.call_args.kwargs["params"]["term"] == "splicing[ti] AND Smith[au]"
    ...
    def test_api_key_sent_in_params(self):
        ...
        with patch("httpx.Client", cls):
            src.search("RNA")
        assert mc.get.call_args.kwargs["params"].get("api_key") == "mykey"
        ...
        "entry": [
            {
                ...
                "link": [
                    {
                        "@ref": "self",
                        "@href": "https://api.elsevier.com/content/abstract/scopus_id/85123456789",
                    },
                    {
                        "@ref": "scopus",
                        "@href": "https://www.scopus.com/inward/record.uri?partnerID=HzOxMe3b&scp=85123456789&origin=inward",
                    },
                ],
            }
        ],
...
class TestScopusAPISource:
    ...
    def test_parses_paper_fields(self):
        ...
        assert (
            p.url
            == "https://www.scopus.com/inward/record.uri?partnerID=HzOxMe3b&scp=85123456789&origin=inward"
        )
        assert p.pdf_url is None
        assert p.is_open_access is True
...
class TestScopusBrowserSource:
    ...
    def test_unavailable_without_session(self):
        with (
            patch("mosaic.auth.has_browser", return_value=True),
    ...
    def test_unavailable_without_browser(self):
        with patch("mosaic.auth.has_browser", return_value=False):
            assert not self._source().available()
